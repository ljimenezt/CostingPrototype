...
public class ScheduledActivitiesAction implements Serializable {
	...
	/**
	 * @return machineTypes: selectedItem list type machine types
	 */
		...
		try {
			...
			if (tiposActivityNames != null) {
				...
				if (ControladorContexto.getFacesContext() != null) {
					this.activitiesAction = ControladorContexto
							.getContextBean(ActivitiesAction.class);
					this.activitiesAction.setItemsActivities(opcionesActivityName);
				}
			}
		} catch (Exception e) {
			ControladorContexto.mensajeError(e);
		...
		try {
			if (this.listActivityMachineTemp != null
				...
				for (ActivityMachine activityMachine : listActivityMachineTemp) {
					...
					costConsumableMachine += activityMachine.getConsumablesCostBudget();
				...
				costConsumableMachine+=selectedActivity.getCostMachinesEqBudget();
	...
	public void deleteActivitiesAndMachine() {
		...
		try {
			...
			Double costMachineBudget=this.selectedActivity.getCostMachinesEqBudget() - this.activityMachine.getConsumablesCostBudget();
	...
	public void validateDuration(FacesContext context, UIComponent toValidate,
		...
		boolean flagValue = (flag != null && "si".equals(flag)) ? true
				: false;
		Double duration = (Double) value;
		Double durationActivity = 0.0d;
		if(!flagValue){
		durationActivity = (Double) ControladorFechas.restarFechas(
				activitiesAction.getSelectedActivities().getInitialDtBudget(),
				activitiesAction.getSelectedActivities().getFinalDtBudget());
		} else {
		durationActivity = (Double) ControladorFechas.restarFechas(
	...
	/**
	 * It will calculate the length considering the difference two dates for activity machine
	 */
